<template>
  <!-- 第一个表 -->
  <div class="itemCont">
    <div class="title">CRC评价类型统计图</div>
    <div id="part2">
      ...
      <div class="chart">
        <Bars :list="chart7" :option="{color:'#A7D2AD'}" :colors="['#B6CBDC','#F1847E','#A79ACF']" />
      </div>
    </div>
  </div>
</template>
<script>
import { get_inst, get_smo,get_crc_evaluate,get_years_list} from "@/api/data_manage";
  ...
  methods: {
    handleSubmit(e) {
      ...
      this.form.validateFieldsAndScroll((err, values) => {
        if (!err) {
          this.self_get_crc_evaluate(values);
        }
      });
    },
    self_get_crc_evaluate({ year,inst_id,smo_id } = { year:"",inst_id: "",smo_id:""}) {
      get_crc_evaluate({ instid: inst_id,smo_id:smo_id,year:year }).then(res => {
        let item = {name:[],data:[]}
